...
The plant, measuring under a centimeter in diameter, consists of a frond and root which can be seen in @figFrond. This enables it to perform photosynthesis on the surface of the water while obtaining nutrients found below. Using asexual reproduction, S. Polyrhiza is able to quickly reproduce and spread across the surface of calm bodies of water. In ideal conditions the plant has been found to be able to double in size is just a few days @refDuckweedConditions.

Photosynthesis in combination with usage of nutrients in the water gives  S. Polyrhiza its fast-growing properties. In photosynthesis plants are to create food from carbon dioxide, light energy, and water, allowing for plant growth @refLightRole. An unbalanced equation for this is shown below:
...
Compared to first reaction, this requires more time before and after having the necessary products to slow down or speed up @refPulsedLighting. This is due to photosynthetic induction and the rate at which the plant can take in Carbon dioxide from the air @refInduction. When these two reactions are combined they create the simplified model of photosynthesis shown in the first reaction. A diagram depicting this is shown in @figReactions.
...
Normally photosynthesis occurs in constant lighting from the sun, but increasingly, plants are  being grown in artificial lighting. Often artificial lighting employs Pulse Width Modulation, or PWM, to control the brightness. This means that lights are quickly turned on and off to make them appear dimmer. This can play a role in plant growth and as duckweed becomes an option for waste water treatment it is necessary to understand optimal growing conditions, including what light is used.

Due to the concentration of the reactants in the light dependent reaction being high and products being low in concentration during periods of darkness, I predict that the when a chloroplast is first exposed to light the light dependent stage of photosynthesis will be more efficient. Additionally after the light is turned off, there will be residual products from the reaction to fuel the light independent stage. This is shown in @figPrediction. I hypothesize that as lighting frequency is increased the growth rate of S. Polyrhiza will also increase due to there being more time spent in the region of higher efficiency.
...
Previous studies have analyzed the effect of pulsed lighting on lettuce and found that higher frequencies did result in increased growth when compared to lower frequencies of light. This effect was found to be most pronounced in frequencies above 1kHz @refPulsedLighting.

This topic is important to me as it connects the increasingly important world of technology with the world of biology. For example, automated systems are now able to effectively grow plants in hydroponic systems.
...
=== LED
The LED chosen had a warmth of 4000k and Color Rendering Index (CRI) of 90. The spectrum of light emitted is shown in @spectrum. This light was chosen as it contains both the wavelengths used in chlorophyll A and chlorophyll B.
#v(1em)
...
=== Spirodela Polyrhiza
The plants was acquired at a local aquarium store as part of a larger collection of floating plants that included, duckweed, dwarf water lettuce, and red root floaters. Spirodela Polyrhiza was separated and stored at 19#sym.degree C in a shallow pool of tap water prior to experiment.
...
=== Tap water
Tap water was obtained from the faucet and was left out in a plastic container for a week prior to experiment in order to remove chlorination.

== Controlled Variables
- *Temperature*: The optimal temperature for S. Polyrhiza is 25$degree$C, but a temperature of 20$degree$C was used to due to limitations in the experimental setup.
...
1. Measure 1000mL of prepared tap water into large container. Using the plastic straw, measure out 0.02ml of the plant supplement. This concentration was found based on the recommended amount of 5ml per 250L. Stir the mixture well using the straw.
...
+ Place containers in a 3 by 5 grid with a spacing of 10 cm between the centers of adjacent containers. Comprehensive diagrams of this setup can be found in @appA. A photo of this can be seen in @figPol.
+ Place LED lighting grid above the containers so that lights are centered above plastic cups.
+ Set five PWM outputs with a 25% duty cycle and a frequency as determined by @grid. Waveforms can be found in @appA.
+ After 24 hours remove LED grid and individually place plastic cups on piece of paper.
+ Take photos from directly above paper with the end of lens 70cm above paper. Beside the cup place a piece of paper identifying the trial number. See @appA for diagrams.
+ Repeat above steps 5, 6, 8, and 9 until three days have passed.
...
== Data Processing
For simplicity the conditions used for frequency will be referred to based on @refer. These are also the meanings of the numbers used in the photos. Additionally, each trial will be referred to with a letter. For each frequency there were 6 trials, so the letters used were A, B, C, D, E, and F.
...
The experimental procedure resulted in 120 raw image files which were than processed in Darktable to create the images found in @appB. A threshold of hue 0 to 61 was used for isolating red. For isolating green a threshold of hue 86 to 172 was used. All pixels within these thresholds were turned into black pixels while all others were turned into white pixels. The resulting images with green and red extracted can be found in @appB.

A Python script was used to count the number of black pixels in all 240 photos generated. These values can be found in @appB. A second Python script was used to take the red pixel count from all photos and generate a single list which was used in @figRed.

Because the tripod likely had small changes in height the lens was a different distance from the paper. This resulted in the red square appearing a different size, and thus a different number of red pixels. Based on the variability in read pixels we are able to determine that the areas acquired from the photos have an error #sym.plus.minus 3% based on the information seen in @figRed.
...
= Results
== Qualitative
After three days had passed under the PWM lighting, plants under different frequencies appeared very similar. All had a bright yellow-green color. Additionally, the area covered by the fronds did not appear to be dramatically larger. Small roots were found at the bottom of the plastic cup having separated from the frond.

== Initial Data Visualization
A color mesh style graph was created to show the total change from $t=0$ to $t=72"hr"$. Due to changes in starting area, an additional visuallization was created to show the percentage change over the course of the three days. These data visualizations can be found in @figMesh.
...
#figure(
  ...
  caption: [Visualization of the change in area (left) and percentage change (right) over the three days of plant growth. See @refer for actual frequency]
)<figMesh>

There does not appear to be any strong relationship and there is large variability in amount grown. Despite this the condition for frequency that appears to have grown the least does appear to be the lowest frequency at 0.01Hz.

The color mesh shown in @figMesh only uses the first and last data but we are able to plot the data acquired from the times in between also. The change in area relative the the starting area for each frequency resulted in the graph shown in @figLine.
...
This graph show a more clear pattern of growth in all frequencies except 0.01Hz. Notably, from day 2 to 3 all frequencies appear to have about the same amount of growth.
...
To better visualize the variability across trials, a bar plot was constructed for each frequency. This is shown in @figBar. All bars overlap, indicating a weak relationship between frequency and growth rate.
...
We will use a standard significance level of p=0.05.

A Python script was used to perform the ANOVA test. This resulted in a p-value of p=0.171. Because this is higher than our significance level of 0.05, we are not able to reject out null hypthesis. This means there is not significant evidence that one of the means is different.

The uncertainty of $plus.minus$3% would not change this result as it would only increase the acquired p-value.


= Conclusion
Using an ANOVA test and a significance value we . The results of this experiment did not yield evidence that frequency of lighting changes the growth rate of S. Polyrhiza. Despite this the frequency that resulted in the lowest average growth across trials was 0.01Hz, potentially indicating potential for future studies.

The wide range of frequencies used did allow for the possiblity to see the change between any two conditions.

= Evaluation
This experiment did face certain limitations which likely contributed to variabilty in the data acquired. One of the most important was the temperature at which the plants were grown at. The thermometer that was placed in the same enclosure as the plants was checked each time the plants were removed and often displayed 18$degree$C. Based on the study of optimal conditions for S. Polyrhiza, this likely led to a lower growth rate, demonstrated in two trials even having negative growth.

Additionally, the time to collect data was limited, resulting in the three day time period that was used. The intensive data collection that necessitated a photo of each plant limited the sustainability of using the same procedure for longer time periods.

The reliablity of using photos and extracting the green areas was less than expected. Often ambient reflections would cause white specks on plant in the photos, resulting in the pixel not being recognized as green. A fairly consistent lighting setup was used, but small changes in the position of the paper and tipod relative to the light did limit the reproducability of the lighting.

Lots of effort did go into the construction of the lighting grid which did function correctly through the entire course of the experiment. Despite this, the original intention was to supply each LED controller PCB with 5V instead of 12V, but I was forced to use 12V due to how the gate driver functioned. This resulted in only being able to use a 50$Omega$ resistor with 100mA of current. Ideally, a lower brightness of the LED would have been used.

In future studies a longer time period could be used with automated data collection using a camera. This would allow for consistent lighting and results that were more independent of the resolution limitations in a photo. Additionally all the plants could be placed in the same tank of water to ensure that they all have access to the same concentration of nutrients required for growth. Similarly, the environment could have been better controlled to maintain optimal temperature and humidity for plant growth. A possible implmentation for this could be a heating mat underneath the plants.
...
== Waveforms
An oscilloscope was used to acquire all waveforms. The waveforms coming from the micrcontroller were measured without an attached load.
...
This script was used to find the difference in starting area and ending area.
#pagebreak()

=== image-analysis-green.py
#sourcefile(read("python-scripts/image-analysis-green.py"), lang: "py")
This script was used to count the number of green pixels.
#pagebreak()

=== image-analysis-red.py
#sourcefile(read("python-scripts/image-analysis-red.py"), lang: "py")
This script was used to count the number of red pixels.
#pagebreak()

=== line-graph-offset.py
#sourcefile(read("python-scripts/line-graph-offset.py"), lang: "py")
This script was used to generate the data used in the line graph.
#pagebreak()

=== pixel-to-area.py
#sourcefile(read("python-scripts/pixel-to-area.py"), lang: "py")
This script was used to convert csv files in pixel units to csv files with $"mm"^2$ units.
#pagebreak()
